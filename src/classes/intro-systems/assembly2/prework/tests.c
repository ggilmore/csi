...
fib(int n);
extern int
index(int* matrix, int rows, int cols, int rindex, int cindex);
extern void
transpose(int* in, int* out, int rows, int cols);
extern float
...
test_fib_3(void)
{
  TEST_ASSERT_EQUAL(2, fib(3));
}
void
test_fib_10(void)
{
  TEST_ASSERT_EQUAL(55, fib(10));
}
void
test_fib_12(void)
{
  TEST_ASSERT_EQUAL(144, fib(12));
}
