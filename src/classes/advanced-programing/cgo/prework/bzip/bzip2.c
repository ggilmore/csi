...
static void
panic(const Char*) NORETURN;
static void
ioError(void) NORETURN;
static void
outOfMemory(void) NORETURN;
static void
configError(void) NORETURN;
static void
crcError(void) NORETURN;
static void cleanUpAndFail(Int32) NORETURN;
static void
compressedStreamEOF(void) NORETURN;

static void
copyFileName(Char*, Char*);
static void* myMalloc(Int32);
static void
applySavedFileAttrToOutputFile(IntNative fd);
...
static void
uInt64_from_UInt32s(UInt64* n, UInt32 lo32, UInt32 hi32)
{
...
/*---------------------------------------------*/
static void
compressStream(FILE* stream, FILE* zStream)
{
  BZFILE* bzf = NULL;
  UChar ibuf[5000];
  Int32 nIbuf;
...
errhandler:
  BZ2_bzWriteClose64(&bzerr_dummy,
                     bzf,
                     1,
                     &nbytes_in_lo32,
                     &nbytes_in_hi32,
...
/*---------------------------------------------*/
static Bool
uncompressStream(FILE* zStream, FILE* stream)
{
  BZFILE* bzf = NULL;
...
cleanUpAndFail(Int32 ec)
{
  ...
  if (srcMode == SM_F2F && opMode != OM_TEST && deleteOutputOnInterrupt) {
    ...
    if (retVal == 0) {
      ...
      fprintf(
        stderr, "%s: WARNING: deletion of output file suppressed\n", progName);
      fprintf(stderr,
...
/*---------------------------------------------*/
static void
mySIGSEGVorSIGBUScatcher(IntNative n)
{
  if (opMode == OM_Z)
    fprintf(
      stderr,
...
*/
static FILE*
fopen_output_safely(Char* name, const char* mode)
{
#if BZ_UNIX
  FILE* fp;
  IntNative fh;
...
static void
applySavedTimeInfoToOutputFile(Char* dstName)
{
#if BZ_UNIX
  IntNative retVal;
...
/*---------------------------------------------*/
static Bool
containsDubiousChars(Char* name)
{
#if BZ_UNIX
...
static Bool
hasSuffix(Char* s, const Char* suffix)
{
  Int32 ns = strlen(s);
...
static Bool
mapSuffix(Char* name, const Char* oldSuffix, const Char* newSuffix)
{
...
compress(Char* name)
{
  ...
  if (srcMode == SM_F2F && !forceOverwrite && notAStandardFile(inName)) {
    if (noisy)
      fprintf(
        stderr, "%s: Input file %s is not a normal file.\n", progName, inName);
    setExit(1);
    return;
  }
  if (srcMode == SM_F2F && fileExists(outName)) {
    if (forceOverwrite) {
      ...
      fprintf(
        stderr, "%s: Output file %s already exists.\n", progName, outName);
      setExit(1);
      return;
    }
  }
...
/*---------------------------------------------*/
static void
addFlagsFromEnvVar(Cell** argList, Char* varName)
{
  Int32 i, j, k;
  Char *envbase, *p;
