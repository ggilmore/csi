...
/*---------------------------------------------*/
static void
tooManyBlocks(Int32 max_handled_blocks)
{
  fprintf(stderr,
...
/*---------------------------------------------*/
static void
bsPutUChar(BitStream* bs, UChar c)
{
  Int32 i;
  for (i = 7; i >= 0; i--)
...
/*---------------------------------------------*/
static void
bsPutUInt32(BitStream* bs, UInt32 c)
{
  Int32 i;
...
endsInBz2(Char* name)
{
  ...
  if (n <= 4)
    return False;
  return (name[n - 4] == '.' && name[n - 3] == 'b' && name[n - 2] == 'z' &&
          name[n - 1] == '2');
}
...
main(Int32 argc, Char** argv)
{
  ...
  while (True) {
    ...
    if (((buffHi & 0x0000ffff) == BLOCK_HEADER_HI &&
      ...
      if (currBlock > 0 && (bEnd[currBlock] - bStart[currBlock]) >= 130) {
        fprintf(stderr,
                "   block %d runs from " MaybeUInt64_FMT " to " MaybeUInt64_FMT
                "\n",
                rbCtr + 1,
                bStart[currBlock],
                bEnd[currBlock]);
  ...
  if (rbCtr < 1) {
    fprintf(
      stderr, "%s: sorry, I couldn't find any block boundaries.\n", progName);
    exit(1);
  };
